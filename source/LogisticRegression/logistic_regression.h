...
class Logistic : public BaseClassifier {
	colvec weight; /*!< Weight is a column vector of coefficients and is evaluated using stochastic
                            gradient descent
                        */
	double alpha; /*!< alpha is a learning rate, which states how much the coeffcients change 
...
public:
	Logistic(double); /**< A logistic regression object is created by a constructor wherein the
                           ...
                           */
	void train(const mat&, const colvec&, uword); /**< The train function accepts a matrix of sample inputs, a column vector of sample labels, and a uword representing the number of epochs. Given these inputs, logistic regression is able to generate a model for predicting on new inputs. */

	void predict(const mat&, colvec&); /**< The predict function accepts a matrix of inputs and a column vector of labels initialized to 0. As the column vector is passed by reference, the algorithm simply inserts the computed predictions into the vector and returns void. */

	colvec sigmoid(const colvec&); /**< The sigmoid function calculates the sigmoid function. The function receives a column vector for input and returns a column vector as a result. The logistic function transforms any input into the range (0, 1). */
